...
{      			    
    ...
    for(t=0;t<16;t++)
		{
				OLED_WR_Byte(Hz5933[2*no][t],OLED_DATA);
			//	adder+=1;
     }	
		OLED_Set_Pos(x,y+1);	
    for(t=0;t<16;t++)
			{	
				OLED_WR_Byte(Hz5933[2*no+1][t],OLED_DATA);
			//	adder+=1;
      }					
}

//显示汉字短语
void OLED_ShowCHinesePhrase(u8 x,u8 y,char* ch,u8 num)
{   
	u8 t,i=0;
	for(i=0;i<num;i++)
	{
    ...
    for(t=0;t<16;t++)
		{
				OLED_WR_Byte(*(ch+16*2*i+t),OLED_DATA);
    }	
		OLED_Set_Pos(x,y+1);	
    for(t=0;t<16;t++)
		{	
				OLED_WR_Byte(*(ch+16*(2*i+1)+t),OLED_DATA);
    }	
